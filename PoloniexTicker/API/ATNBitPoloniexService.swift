...
class ATNBitPoloniexService {
    ...
    typealias TickerUpdate = (Ticker)->Void
	...
	init() {
		...
		socket.onText = { (text: String) in
			...
			if let tickerinfoAny = json??[2] as? [Any] {
				...
				let newTicker = Ticker.init(id: Int(tickerinfo[0]) ?? 0, last: tickerinfo[1], lowestAsk: tickerinfo[2], highestBid: tickerinfo[3], percentChange: tickerinfo[4], baseVolume: tickerinfo[5], quoteVolume: tickerinfo[6], isFrozen: tickerinfo[7], high24hr: tickerinfo[8], low24hr: tickerinfo[9])
				self.notifyObservers(updates:newTicker)
			}
		}
	...
	func notifyObservers(updates ticker:Ticker) {
		for (_,observer) in observers {
			observer.receivedUpdates(ticker)
		}
	}
